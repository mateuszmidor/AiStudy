...
type Message struct {
	...
	Content   string     `json:"content"`              // either GPT prompt (request) or GPT answer (response)
	...
	for _, choice := range gptResp.Choices {
		...
		for _, call := range choice.Message.ToolCalls {
			...
			case "get_temperature_at_location_in_celsius":
				// TODO: pass back function call result to the model: https://youtu.be/p0I-hwZSWMs?si=4XnsT_TgBJpwelLS&t=984
